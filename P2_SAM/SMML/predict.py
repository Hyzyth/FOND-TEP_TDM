...
def test_softmax(
    ...
    for i, data in enumerate(test_loader):
        ...
        model1.module.is_training=False
        model2.module.is_training=False
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    x_input = x[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size]
                    pred_part1 = model1(x_input, mask)
                    pred_part2 = model2(x_input, mask)
                    # pred_part = cal_confidence(pred_part1, pred_part2)
                    pred_part = torch.mean(torch.stack([pred_part1, pred_part2], dim=1), dim=1)
                    # soft = torch.nn.Softmax(dim=1)
                    # pred_part = soft(pred_part)
                    pred[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size] += pred_part
        pred = pred / weight
        b = time.time()
        pred = pred[:, :, :H, :W, :T]
        pred = torch.argmax(pred, dim=1)
        if not os.path.exists(os.path.join(save_path,mask_name)):
            os.makedirs(os.path.join(save_path,mask_name))
        # print(mask_name)
        # exit()
        # print(x.shape, x_input.shape, pred.shape, target.shape)
        medio.save(pred.squeeze(0).cpu().numpy().astype(np.uint8), os.path.join(save_path, mask_name, str(names[0]) + '_seg.nii.gz'))
        medio.save(target.squeeze(0).cpu().numpy().astype(np.uint8), os.path.join(save_path, mask_name, str(names[0]) + '_gt.nii.gz'))
