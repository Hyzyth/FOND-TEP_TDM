...
class Brats_loadall_nii(Dataset):
    def __init__(self, transforms='', root=None, modal='all', num_cls=4, train_file='train.txt'):
        ...
        # n_fold_list = np.array_split(pid_idx, 3)
    ...
    def __getitem__(self, index):
        ...
        y = np.load(segpath)
        samsegpath = volpath.replace('vol', 'samseg')
        y_s = np.load(samsegpath) # k, m,h,w,z
        k,m,h,w,z = y_s.shape
        y_s = y_s.reshape(-1, h,w,z)
 
        x, y = x[None, ...], y[None, ...] # 1,h,w,z,m
        ...
        y_s = torch.from_numpy(y_s) # m*k,h,w,z
        
        mask_idx1, mask_idx2 = np.random.choice(15, 2, replace=False)
        mask1 = torch.squeeze(torch.from_numpy(mask_array[mask_idx1]), dim=0)
        mask2 = torch.squeeze(torch.from_numpy(mask_array[mask_idx2]), dim=0)
        return x, yo, y_s, mask1, mask2, name
