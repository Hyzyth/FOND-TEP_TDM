...
hecktor_dataset.py
==================
HECKTOR-specific dataset classes that extend the generic NPZ raw dataset
to handle dual-modality (CT + PET) input for head-and-neck tumour segmentation.

The HECKTOR NPZ files produced by ``data_preparation/prepare_hecktor_npz.py``
contain separate ``ct_imgs`` and ``pet_imgs`` arrays (both uint8, shape D×H×W).
This module fuses them into a 3-channel representation [CT, PET, PET] that
SAM2's ImageNet-pretrained ViT backbone can process.

Label convention in ``gts``
----------------------------
    0 – background
...
class HECKTORNPZRawDataset(VOSRawDataset):
    """Dataset loader for HECKTOR NPZ files with dual-modality (CT + PET) input.

    Each NPZ file contains:
        ``ct_imgs``  – (D, H, W) uint8, CT windowed to [0, 255]
        ``pet_imgs`` – (D, H, W) uint8, PET SUV normalised to [0, 255]
        ``gts``      – (D, H, W) uint8, 0/1/2 label mask
    ...
    ----------
    folder : str
        Path to the directory containing NPZ files (may include sub-folders).
    file_list_txt : str, optional
        Path to a plain-text file listing allowed NPZ basenames (one per line).
        When ``None`` all NPZ files under *folder* are used.
    excluded_videos_list_txt : str, optional
        Path to a plain-text file of NPZ basenames to exclude.
    sample_rate : int
        Keep every *sample_rate*-th slice (default 1 = all slices).
    truncate_video : int
        If > 0, keep only the first *truncate_video* slices.
    """
        ...
        for root, _, files in os.walk(folder):
            for fname in files:
                if fname.endswith(".npz"):
                    rel = os.path.relpath(
                        os.path.join(root, fname), folder
                    )
    ...
    def get_video(self, idx: int):
        """Load one NPZ file and return a ``(VOSVideo, NPZSegmentLoader)`` pair.

        Parameters
        ----------
        idx : int
            Index into ``self.video_names``.
        ...
        # Build per-slice 3-channel tensors [CT, PET, PET] ∈ [0, 1].
    ...
    def _fuse_modalities(ct_slice: np.ndarray, pet_slice: np.ndarray) -> torch.Tensor:
        """Fuse a single CT and PET axial slice into a 3-channel float tensor.

        The resulting tensor has shape (3, H, W) with values in [0, 1].
        Channel layout: [CT, PET, PET].

        Parameters
        ----------
        ct_slice  : np.ndarray  (H, W) uint8
        pet_slice : np.ndarray  (H, W) uint8
        ...
        # Stack as [CT, PET, PET] to fill the 3 RGB channels.
