...
#     epoch: int,
#     max_epochs: int,
#     warmup: int = 6,         # give seg‐head 10 epochs to learn pure source
#     initial: float = 0.1,     # jump to 0.1 at epoch==warmup
#     ceiling: float = 0.3,     # maximum α
#     steepness: float = 3.0    # gentler slope
# ) -> float:
#     if epoch < warmup:
#         return 0.0
#     # normalized progress after warmup
#     p = (epoch - warmup) / float(max_epochs - warmup)
#     # a mild logistic ramp from 0→1
#     ramp = 1.0 / (1.0 + np.exp(-steepness * (p - 0.5)))
#     # scale into [initial → ceiling]
#     return float(np.clip(initial + (ceiling - initial) * ramp, initial, ceiling))


# def lambda_domain_schedule(
#     epoch: int,
#     max_epochs: int,
#     warmup: int = 6,         # same warmup
#     initial: float = 0.05,    # a small domain‐weight step
#     ceiling: float = 0.2,     # maximum λ
#     steepness: float = 3.0
# ) -> float:
#     if epoch < warmup:
#         return 0.0
    ...
    with tqdm(range(num_steps), desc=f"DomainAdapt-Train Epoch {epoch + 1}", leave=False) as progress_bar:
        for step_idx in progress_bar:
            ...
            with autocast("cuda"):
                ...
                total_loss_s = seg_loss_s + float(lambda_domain) * domain_loss_s
            total_seg_loss += seg_loss_s.item()

            with autocast("cuda"):
                _, domain_loss_t, pooled_feat_t = model(
                    images_t,
                    seg_labels=None,
                ...
                total_loss_t = float(lambda_domain) * domain_loss_t
...
def run(
    ...
    if cfg.training.resume:
        ...
        pkl_path = os.path.join("/home/monetai/Desktop/dillan/allBrats/code/brrr/Brain-Tumors-Segmentation/archiveModels/t2_t1/t2_t1_initial/best-model/best-model.pkl")
...
def main(cfg: DictConfig):
    ...
    if rank == 0:
        ...
        logger.info(f"Source dataset size={len(ds_source)}, Target dataset size={len(ds_target)}, "
