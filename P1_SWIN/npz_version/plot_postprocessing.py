...
# Removed "GTVp Shell" from the palette
PALETTE = {
...
def generate_plots(csv_path: str, output_dir: str):
    ...
    # Removed "shell_removed_GTVp_mm3"
    vol_cols = [
    ...
    if "timepoint" in df.columns and df["timepoint"].nunique() > 1:
        ...
        # Changed subplots size from 3 to 2 because we only have 2 categories of removal per class now
        fig, axes = plt.subplots(1, len(label_map.values()), figsize=(16, 5), sharey=False)
    ...
    # Removed "shell_removed_GTVp_count"
    count_cols = [
